...
var a = 1;
function outer() {/**--------(3) inner()함수의 실행 컨텍스트가 콜 스택의 위 담기면 
                            outer()컨텍스트와 관련된 코드의 실행을 중단하고 
                            inner()내부의 코드를 순서대로 실행한다. */
    function inner() {
...
outer();    /**--------(2) 전역 컨텍스트와 관련된 코드들을 순차로 진행하다가 outer()함수를 호출하면 
                    ...
                    콜 스택 맨 위에 outer실행 컨텍스트가 놓인 상태가 됐으므로 
                    전역 컨텍스트와 관련된 코드의 실행을 일시 중단하고 
...
/** 
 * 전역 - outer() - inner() - console.log(a) - var a=3 - console.log(a) - console.log(a) 
 * */
